...
import { User, Role, JobTitle } from '../models';
...
module.exports = {
  async create(req, res) {
    ...
    const jobTitleId = _.get(req, 'body.jobTitle.id', 0);
  ...
  async list(req, res) {
    const users = await User.findAll(
      userQuery.list( { req, User, Role, JobTitle } )
    );
  ...
  async get(req, res) {
    const user = await User.findByPk(
      req.params.userId,
      userQuery.get( { req, User, Role, JobTitle } )
    );
  ...
  async update(req, res) {
    ...
    const updatedUser = await user.update( {
      ...
      jobTitleId: _.get(req, 'body.jobTitle.id', user.jobTitleId),
