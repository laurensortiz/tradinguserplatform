  ...
  async create(req, res) {
    ...
    try {
      await ORM.transaction(async (t) => {
        ...
        if (!userAccount) {
          throw new Error(
            "Ocurrió un error al momento de buscar la cuenta del usuario"
          );
        }
        ...
        const guaranteeOperation = ToFixNumber(
          Number(userAccount.guaranteeOperation) -
            (Number(req.body.maintenanceMargin) +
      ...
      try {
        await marketOperation.update(
          {
            ...
            userAccountId:
              _.get(req, "body.userAccount.id", 0) ||
              marketOperation.userAccountId,
            brokerId:
              _.get(req, "body.broker.id", 0) || marketOperation.brokerId,
            productId:
              _.get(req, "body.product.id", 0) || marketOperation.productId,
            commoditiesTotal:
              req.body.commoditiesTotal || marketOperation.commoditiesTotal,
            commodityId:
              _.get(req, "body.commodity.id", 1) || marketOperation.commodityId,
            assetClassId:
              _.get(req, "body.assetClass.id", 1) ||
              marketOperation.assetClassId,
            ...
            maintenanceMargin:
              req.body.maintenanceMargin || marketOperation.maintenanceMargin,
            amount: req.body.amount || marketOperation.amount,
            initialAmount:
              req.body.initialAmount || marketOperation.initialAmount,
            ...
            holdStatusCommission:
              req.body.holdStatusCommission ||
              marketOperation.holdStatusCommission,
        ...
        if (marketOperation.status === 4) {
          ...
          const commission = isProfitPositive
            ? ToFixNumber(
                Number(((profit * Number(percentage)) / 100).toFixed(2))
              )
            : 0;
          const hold =
            isProfitPositive && !isBrokerGuarantee
              ? GetHoldCommissionAmount(profit)
              : 0;
          const endProfit = ToFixNumber(profit - commission - hold);
          // Close Calculations
          const marginUsed = ToFixNumber(
            ((Number(initialAmount) + Number(maintenanceMarginAmount)) * 10) /
              100
          );
          ...
          const accountGuaranteeEndOperation = ToFixNumber(
            Number(userAccount.guaranteeOperation) +
              Number(guaranteeOperationProduct)
          );
          ...
          case "status":
            if (updateValue === 4) {
              ...
              for (const operationID of operationsIds) {
                ...
                if (!marketOperation) {
                  throw new Error(
                    "Ocurrió un error al momento de buscar la operación"
                  );
                }
                ...
                if (marketOperation.status === 4) {
                  throw new Error(
                    "Una o más operaciones seleccionas ya se encuentran cerradas"
                  );
                }
                ...
                // Close Calculations
                const marginUsed = ToFixNumber(
                  ((Number(initialAmount) + Number(maintenanceMarginAmount)) *
                    10) /
                    100
                );
                const guaranteeOperationProduct = ToFixNumber(
                ...
                try {
                  ...
                  const updatedUserAccountSnapShot = JSON.stringify(
                    updatedUserAccount
                  );
            ...
            result = await Promise.all(
              operationsIds.map(async (operationID) => {
                ...
                if (marketOperation.status !== 1) {
                  throw new Error(
                    "Una o más operaciones seleccionadas no se encuentran Activas"
                  );
                }
                 ...
                 */
                const gpAmount = Number(
                  _.get(updateValue, "gpAmount", 0).replace(/\,/g, "")
                );
                const marketPrice = Number(
                  _.get(updateValue, "marketPrice", 0).replace(/\,/g, "")
                );
                const commoditiesTotal = Number(
                  marketOperation.commoditiesTotal
                );
            ...
            result = await Promise.all(
              operationsIds.map(async (accountId, index) => {
                ...
                try {
                  ...
                  if (index === 0) {
                    nextOrderId =
                      Number(lastMarketOperationEntry[0].orderId) + 1;
                  } else {
                  ...
                  const marketOperation = await MarketOperation.create(
                    {
                      ...
                      holdStatusCommission:
                        updateValue.holdStatusCommission || 0,
