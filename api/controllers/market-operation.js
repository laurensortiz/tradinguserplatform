...
module.exports = {
  ...
  async bulkUpdate(req, res) {
    ...
    let transaction
    logger.info(
      `🚀 Bulk Update [${transactionID}] - Market Operation - User: ${userId} - UpdateType: ${updateType}  - Operation Size: ${operationsIds.length}`
    )
    ...
    try {
      ...
      logger.error(
        `🔥 Bulk Update [${transactionID}] - Market Operation - User: ${userId} - Error: ${err}`
      )
      ...
      const endTime = performance.now()
      logger.info(
        `⏰  Bulk Update [${transactionID}] - Market Operation - User: ${userId} - Time: ${millisecondsToSeconds(
          endTime - startTime
        )}`
      )
    }
