...
function BulkUpdateSteps({
  ...
  const _bulkUpdateValue = () => {
    switch (updateScope) {
      ...
      default:
        return (
          <div className="no-visible" style={{ width: 150, height: 40 }} />
        );
    }
  };
  ...
  const _lastStepContent = () => {
    const title = isProcessComplete ? (
      isBulkSuccess ? (
        <span>
          <Tag color="#046d11">{selectedElements}</Tag>Productos actualizados
          correctamente
        </span>
      ) : (
        "Ocurrió un error"
      ...
      <span>
        Proceder con la actualización de{" "}
        <Tag color="#046d11">{selectedElements}</Tag>productos
      </span>
    );
    const status = isProcessComplete
      ? isBulkSuccess
        ? "success"
        : "error"
      : "info";
      ...
      <Result
        ...
        extra={
          <Button
            ...
            key="console"
            onClick={() =>
              onClickUpdate({ updateType, updateValue, updateScope })
            }
  ...
  const steps = [
    {
      ...
      content: (
        <h3>
          ...
          <Tag color="#046d11">{selectedElements}</Tag>
        </h3>
      ),
    },
    {
