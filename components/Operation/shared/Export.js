...
class Export extends PureComponent {
  ...
  _getAccountTemplateMarket = (data) => {
    ...
    return [
      ...
      [ `Garantías disponibles: ${FormatCurrency.format( data.userAccount.guaranteeOperation )}`, `Saldo Inicial: ${FormatCurrency.format( data.userAccount.balanceInitial )}`, `Margen de mantenimiento: ${FormatCurrency.format( data.userAccount.maintenanceMargin )}` ],
      [ '' ],
      ...
      [`Producto: ${data.product.name}`, `Lotage: ${data.commoditiesTotal} [${data.commodity.name}](${data.assetClass.name})`, `Precio de compra: ${FormatCurrency.format(data.buyPrice)}`],
