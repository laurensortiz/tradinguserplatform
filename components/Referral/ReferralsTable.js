...
class ReferralsTable extends Component {
  ...
  _onSelectBulkReport = () => {
    const exportData = _.isEmpty(this.state.currentDataSource) ? this.state.referrals : this.state.currentDataSource;
    const selectedReferrals = _.filter(exportData, referral => _.includes(this.state.selectedRowKeys, referral.id))
