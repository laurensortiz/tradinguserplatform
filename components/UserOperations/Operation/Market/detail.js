...
class Detail extends PureComponent {
  render() {
    ...
    const holdStatusCommission = _.get(
      this.props,
      'currentOperation.holdStatusCommissionEndOperation',
      '0.00'
    )
          ...
          <Col>
            <Descriptions title={t('title operationInformation')}>
              <Descriptions.Item label={t('product')}>
                <Tag className="product-tag">{productName}</Tag>
              </Descriptions.Item>
              <Descriptions.Item label={t('createdAt')}>{FormatDate(startDate)} </Descriptions.Item>
              <Descriptions.Item label={t('investment')}>
              ...
              <Descriptions.Item label={t('maintenanceMargin')}>
                {FormatCurrency.format(maintenanceMargin)}
              </Descriptions.Item>
              <Descriptions.Item label={t('lotage')}>
                {commoditiesTotal} <Tag>{commodityName}</Tag>{' '}
                <Tag className={`asset-class ${AssetClassColor(assetClassName).name}`}>
                  {assetClassName}
                </Tag>
              </Descriptions.Item>
              ...
              <Descriptions.Item label={t('commissionStatusHold')}>
                {FormatCurrency.format(holdStatusCommission)}
              </Descriptions.Item>
            </Descriptions>
          </Col>
        </Row>
