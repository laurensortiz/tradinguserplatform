        ...
        <Row>
          <Col>
            <Descriptions title={t('accountInformation')}>
              <Descriptions.Item label={t('accountValue')}><span className={IsOperationPositive(accountValue, balanceInitial) ? 'positive txt-highlight' : 'negative txt-highlight'}>{FormatCurrency.format(accountValue)}</span> </Descriptions.Item>
              ...
              {_.isEqual(associatedOperation, 1) ? (
                <Descriptions.Item label={ t('availableGuarantees') }>{FormatCurrency.format(guaranteeOperation)}</Descriptions.Item>
              ) : null}
              <Descriptions.Item label={t('initialAmount') }>{FormatCurrency.format(balanceInitial)}</Descriptions.Item>
              <Descriptions.Item label={ t('guaranteesCredits') }>{FormatCurrency.format(guaranteeCredits)}</Descriptions.Item>
              {_.isEqual(associatedOperation, 1) ? (
                <>
                <Descriptions.Item label={`${t('marginUsed')} 10%`}>{FormatCurrency.format(marginUsed)}</Descriptions.Item>
                <Descriptions.Item label={ t('commissionsByReference') }>{FormatCurrency.format(commissionByReference)}</Descriptions.Item>
                </>
              ) : null}
            </Descriptions>
          </Col>
