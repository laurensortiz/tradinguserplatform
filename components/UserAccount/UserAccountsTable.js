    ...
    if (bulkOperation.updateType === 'report') {
      ...
      if (!_.isEmpty(invalidAccount)) {
        notification.error({
          ...
          description: `Los siguientes usuarios no cuentas con fondos suficientes para la colocación solicitada: ${invalidAccount.join(
            ', '
          )}`,
        })
      } else {
      ...
      <Col>
        <h3>
          ...
          <Tag color="#1b1f21" style={{ fontSize: 14, marginLeft: 10 }}>
            {_.size(this.state.userAccounts)}
          </Tag>
        </h3>
      </Col>
    </Row>
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (data) => (
          <span
            className={
              IsOperationPositive(data.accountValue, data.balanceInitial)
                ? 'positive txt-highlight'
                : 'negative txt-highlight'
            }
            key={data.accountValue}
          >
            {DisplayTableAmount(data.accountValue)}
          </span>
        ),
