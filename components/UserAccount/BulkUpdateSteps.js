...
function BulkUpdateSteps({
      ...
      case 'create-operation':
        return (
          <AddOperationForm
            ...
            onRequestSaveOperation={_onRequestSaveOperation}
          />
        )
      default:
  ...
  const _lastStepContent = () => {
    const title = isProcessComplete ? (
      isBulkSuccess ? (
        <span>
          <Tag color="#046d11">{selectedElements}</Tag>Cuentas actualizadas correctamente
        </span>
      ) : (
        'Ocurrió un error'
      )
      ...
      <span>
        Proceder con la actualización de <Tag color="#046d11">{selectedElements}</Tag>cuentas
      </span>
    )
  ...
  const steps = [
    {
      ...
      content: (
        <h3>
          Cuentas seleccionadas <Tag color="#046d11">{selectedElements}</Tag>
        </h3>
      ),
    },
    {
