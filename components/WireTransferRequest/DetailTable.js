...
class DetailTable extends Component {
  ...
  _handleExportHistory = (wireTransferRequestId) => {
    const selectedWireTransferRequest = _.find(this.state.wireTransferRequests, {id: wireTransferRequestId})
  ...
  _onSelectBulkReport = () => {
    const exportData = _.isEmpty(this.state.currentDataSource) ? this.state.wireTransferRequests : this.state.currentDataSource;
    const selectedWireTransferRequests = _.filter(exportData, wireTransferRequest => _.includes(this.state.selectedRowKeys, wireTransferRequest.id))
      ...
      <Col sm={ 12 }>
        { this.state.isBulkActive ? (
          ...
          <Button size="large" type="secondary" style={ { float: 'right' } } onClick={ () => this.setState( { isBulkActive: true } ) }>
            <Icon type="interaction"/> <span>Generar Reporte</span></Button>
        ) }
  ...
  _displayTableFooter = () => (
    <Row>
      <Col>
        <h3>Total de WireTransferRequests: <Tag color="#1b1f21" style={{fontSize: 14, marginLeft: 10}}>{_.size(this.state.wireTransferRequests)}</Tag></h3>
      </Col>
    </Row>
  )
  ...
  render() {
    ...
    const datesInTimes = _.map( this.state.wireTransferRequests, record => moment( record.createdAt ) ),
